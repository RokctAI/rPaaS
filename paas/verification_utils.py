...
def generate_verification_code(order_id, amount, shop_id):
    ...
    # Take the first 4 bytes and convert to big-endian integer (matching Flutter)
    hash_int = int.from_bytes(digest[:4], byteorder='big')
    
    # Use modulo and absolute value to get 5 digits, pad with zeros
    # Note: Python's int.from_bytes from big endian is always positive, but we use abs for safety
