...
def modify_project_files(temp_dir, app_config):  # noqa: C901
    ...
    if os.path.exists(build_gradle_path):
        with open(build_gradle_path, "r") as f:
            content = f.read()
        content = re.sub(
            r'applicationId ".*"', f'applicationId "{app_config.package_name}"', content
        )
    ...
    if os.path.exists(manifest_path):
        with open(manifest_path, "r") as f:
            content = f.read()
        content = re.sub(
            r'package=".*"', f'package="{app_config.package_name}"', content
        )
...
def _generate_flutter_app(app_config_name):  # noqa: C901
    ...
    try:
        ...
        log_message(
            f"Running build command: {' '.join(build_command)}...", app_config.name
        )
        env = os.environ.copy()
        flutter_path = settings.flutter_sdk_path
        env["PATH"] = f"{flutter_path}/bin:{env['PATH']}"
        ...
        file_doc = frappe.new_doc(
            "File",
            {
                "file_name": f"{app_config.name.replace(' ', '_')}-{
