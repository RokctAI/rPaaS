...
class TestShopAPI(FrappeTestCase):
    def setUp(self):
        ...
        if not frappe.db.exists("Shop", {"shop_name": "Test Shop 1"}):
            ...
            self.shop1 = {
                "data": frappe.get_doc("Shop", {"shop_name": "Test Shop 1"}).as_dict()
            }
        ...
        if not frappe.db.exists("Shop", {"shop_name": "Test Shop 2"}):
            ...
            self.shop2 = {
                "data": frappe.get_doc("Shop", {"shop_name": "Test Shop 2"}).as_dict()
            }
        ...
        if not frappe.db.exists("Shop", {"shop_name": "Test Shop 3 Not Approved"}):
            ...
            self.shop3_not_approved = {
                "data": frappe.get_doc(
                    "Shop", {"shop_name": "Test Shop 3 Not Approved"}
                ).as_dict()
            }
        ...
        if not frappe.db.exists("Shop", {"shop_name": "Test Shop 4 Not Visible"}):
            ...
            self.shop4_not_visible = {
                "data": frappe.get_doc(
                    "Shop", {"shop_name": "Test Shop 4 Not Visible"}
                ).as_dict()
            }
    ...
    def test_get_shops_pagination(self):
        ...
        all_shops_response = get_shops(
            limit_page_length=100, order_by="shop_name", order="asc"
        )
        all_shops = all_shops_response.get("data")

        # Find index of Test Shop 1
        index_1 = next(
            (i for i, s in enumerate(all_shops) if s["id"] == "Test Shop 1"), -1
        )
        # Find index of Test Shop 2
        index_2 = next(
            (i for i, s in enumerate(all_shops) if s["id"] == "Test Shop 2"), -1
        )
    ...
    def test_get_shop_details_success(self):
        ...
        self.assertEqual(
            shop_details["translation"]["title"], self.shop1.get("data")["shop_name"]
        )
    ...
    def test_get_shops_ordering(self):
        ...
        our_shops_asc = [
            s for s in shops_asc if s["id"] in ["Test Shop 1", "Test Shop 2"]
        ]
