...
frappe.provide("{app_name}.erp.tenant.stock.delivery_note");
		...
		if (
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
erpnext.stock.DeliveryNoteController = class DeliveryNoteController extends (
		...
		if (
			...
			this.frm.add_custom_button(
				...
				function () {
					...
					erpnext.utils.map_current_doc({
						method: "{app_name}.erp.doctype.pick_list.pick_list.create_dn_for_pick_lists",
						...
						get_query_method: "{app_name}.erp.doctype.pick_list.pick_list.get_pick_list_query",
						size: "extra-large",
						allow_child_item_selection: true,
		...
		if (!doc.is_return && doc.status != "Closed") {
			...
			if (
				...
				this.frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm,
						});
					},
					__("Create")
	...
	make_shipment() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_installation_note() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_installation_note",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	make_delivery_trip() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm,
		});
	}
	...
	update_status(status) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.delivery_note.delivery_note.update_delivery_note_status",
