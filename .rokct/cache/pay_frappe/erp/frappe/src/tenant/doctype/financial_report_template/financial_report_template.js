...
async function refresh_tree_view(dialog, account_rows) {
	...
	const { message: filtered_accounts } = await frappe.call({
		method: "{app_name}.erp.doctype.financial_report_template.financial_report_engine.get_filtered_accounts",
	...
	const tree = new FilteredTree({
		...
		label: company,
		root_value: company,
		method: "{app_name}.erp.doctype.financial_report_template.financial_report_engine.get_children_accounts",
