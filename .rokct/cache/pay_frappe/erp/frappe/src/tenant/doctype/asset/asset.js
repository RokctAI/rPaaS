...
frappe.ui.form.on("Asset", {
	...
	setup: function (frm) {
		...
		frm.make_methods = {
			"Asset Movement": () => {
				frappe.call({
					method: "{app_name}.erp.doctype.asset.asset.make_asset_movement",
					freeze: true,
					args: {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code },
			};
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code },
			};
		});
	...
	refresh: async function (frm) {
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.asset_type == "Composite Asset") {
				frappe.call({
					method: "{app_name}.erp.doctype.asset.asset.has_active_capitalization",
					args: {
						asset: frm.doc.name,
					},
					callback: function (r) {
	...
	make_journal_entry: function (frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.asset.asset.make_journal_entry",
			args: {
	...
	setup_chart_and_depr_schedule_view: async function (frm) {
		...
		if (frm.doc.calculate_depreciation) {
			...
			let asset_depr_schedule_doc = (
				await frappe.call(
					"{app_name}.erp.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
					{
						asset_name: frm.doc.name,
						status: "Active",
	...
	set_finance_book: function (frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.asset.asset.get_item_details",
			args: {
	...
	create_asset_maintenance: function (frm) {
		frappe.call({
			...
			method: "{app_name}.erp.doctype.asset.asset.create_asset_maintenance",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_repair: function (frm) {
		frappe.call({
			...
			method: "{app_name}.erp.doctype.asset.asset.create_asset_repair",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_capitalization: function (frm) {
		frappe.call({
			...
			method: "{app_name}.erp.doctype.asset.asset.create_asset_capitalization",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	sell_asset: function (frm) {
		const make_sales_invoice = (sell_qty) => {
			frappe.call({
				method: "{app_name}.erp.doctype.asset.asset.make_sales_invoice",
				args: {
					asset: frm.doc.name,
					item_code: frm.doc.item_code,
					company: frm.doc.company,
	...
	split_asset: function (frm) {
		...
		dialog.set_primary_action(__("Split"), function () {
			...
			frappe.call({
				...
				method: "{app_name}.erp.doctype.asset.asset.split_asset",
				callback: function (r) {
	...
	create_asset_value_adjustment: function (frm) {
		frappe.call({
			...
			method: "{app_name}.erp.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	set_values_from_purchase_doc: (frm, doctype) => {
		frappe.call({
			method: "{app_name}.erp.doctype.asset.asset.get_values_from_purchase_doc",
			args: {
	...
	var scrap_dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			frappe.call({
				...
				method: "{app_name}.erp.doctype.asset.depreciation.scrap_asset",
				callback: function (r) {
					frm.reload_doc();
					scrap_dialog.hide();
				},
...
erpnext.asset.restore_asset = function (frm) {
	frappe.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		frappe.call({
			...
			method: "{app_name}.erp.doctype.asset.depreciation.restore_asset",
