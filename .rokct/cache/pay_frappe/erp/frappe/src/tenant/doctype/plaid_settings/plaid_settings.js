	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			...
			frm.add_custom_button(__("Sync Now"), () => {
				frappe.call({
					method: "{app_name}.erp.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
	...
	plaid_success(token, response) {
		...
		frappe.prompt(
			...
			(data) => {
				...
				frappe
					.xcall(
						"{app_name}.erp.doctype.plaid_settings.plaid_settings.add_institution",
						{
							token: token,
							response: response,
						}
					...
					.then((result) => {
						frappe.xcall(
							"{app_name}.erp.doctype.plaid_settings.plaid_settings.add_bank_accounts",
							{
								response: response,
								bank: result,
								company: me.company,
