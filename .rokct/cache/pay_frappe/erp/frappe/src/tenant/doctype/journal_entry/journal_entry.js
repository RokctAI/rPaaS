	...
	make_inter_company_journal_entry: function (frm) {
		...
		d.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				...
				method: "{app_name}.erp.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
	...
	voucher_type: function (frm) {
		...
		if (
			...
			if (["Bank Entry", "Cash Entry"].includes(frm.doc.voucher_type)) {
				return frappe.call({
					type: "GET",
					method: "{app_name}.erp.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
						account_type:
...
erpnext.accounts.JournalEntry = class JournalEntry extends frappe.ui.form.Controller {
	...
	setup_queries() {
		...
		me.frm.set_query("party_type", "accounts", function (doc, cdt, cdn) {
			...
			return {
				query: "{app_name}.erp.doctype.party_type.party_type.get_party_type",
				filters: {
					account: row.account,
				},
			};
		...
		me.frm.set_query("reference_name", "accounts", function (doc, cdt, cdn) {
			...
			if (jvd.reference_type === "Journal Entry") {
				...
				return {
					query: "{app_name}.erp.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party,
					},
	...
	get_outstanding(doctype, docname, company, child) {
		...
		return frappe.call({
			method: "{app_name}.erp.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args },
			callback: function (r) {
				if (r.message) {
...
frappe.ui.form.on("Journal Entry Account", {
	party: function (frm, cdt, cdn) {
		...
		if (!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "{app_name}.erp.doctype.journal_entry.journal_entry.get_party_account_and_currency",
				child: d,
				args: {
					company: frm.doc.company,
...
$.extend(erpnext.journal_entry, {
	...
	set_exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
	...
	reverse_journal_entry: function (frm) {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: frm,
		});
	},
});
...
$.extend(erpnext.journal_entry, {
	set_account_details: function (frm, dt, dn) {
		...
		if (d.account) {
			...
			return frappe.call({
				method: "{app_name}.erp.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
				args: {
					account: d.account,
