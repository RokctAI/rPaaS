...
frappe.ui.form.on("Payment Entry", {
	...
	setup: function (frm) {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "{app_name}.erp.tenant.controllers.queries.get_payment_terms_for_references",
					filters: {
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			...
			return {
				query: "{app_name}.erp.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	party_type: function (frm) {
		...
		frm.set_query("party", function () {
			if (frm.doc.party_type == "Employee") {
				return {
					query: "{app_name}.erp.tenant.controllers.queries.employee_query",
				};
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			...
			return frappe.call({
				method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	set_account_currency_and_balance: function (frm, account, currency_field, callback_function) {
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		return frappe.call({
			method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			frappe.call({
				method: "{app_name}.erp.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
	...
	before_cancel: function (frm) {
		return new Promise((resolve, reject) => {
			frappe.call({
				method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_linked_bank_transactions",
...
frappe.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
					reference_doctype: row.reference_doctype,
					reference_name: row.reference_name,
					party_account_currency:
...
function get_company_defaults(company) {
	return frappe.call({
		method: "{app_name}.erp.doctype.payment_entry.payment_entry.get_company_defaults",
		args: {
			company: company,
		},
	});
