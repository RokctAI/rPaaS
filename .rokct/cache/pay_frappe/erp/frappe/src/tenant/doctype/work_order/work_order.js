...
frappe.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("bom_no", function () {
			if (frm.doc.production_item) {
				return {
					query: "{app_name}.erp.tenant.controllers.queries.bom",
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "{app_name}.erp.doctype.work_order.work_order.get_bom_operations",
				filters: {
		...
		frm.set_query("sales_order", function () {
			if (frm.doc.production_item) {
				return {
					query: "{app_name}.erp.doctype.work_order.work_order.query_sales_order",
					filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.docstatus == 0 && frm.doc.bom_no) {
			frappe.call({
				method: "{app_name}.erp.doctype.work_order.work_order.check_if_scrap_warehouse_mandatory",
				args: {
	...
	create_stock_return_entry: function (frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.work_order.work_order.make_stock_return_entry",
			args: {
	...
	make_job_card: function (frm) {
		...
		const dialog = frappe.prompt(
			...
			function () {
				...
				frappe.call({
					method: "{app_name}.erp.doctype.work_order.work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		erpnext.work_order
			...
			.then((data) => {
				...
				return frappe.xcall("{app_name}.erp.doctype.work_order.work_order.make_stock_entry", {
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method: "{app_name}.erp.doctype.work_order.work_order.get_item_details",
				args: {
...
frappe.ui.form.on("Work Order Item", {
	...
	source_warehouse: function (frm, cdt, cdn) {
		...
		if (!row.item_code) {
			...
			frappe.call({
				method: "{app_name}.erp.tenant.stock.utils.get_latest_stock_qty",
				args: {
	...
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "{app_name}.erp.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company,
				},
...
erpnext.work_order = {
	...
	set_default_warehouse: function (frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			frappe.call({
				method: "{app_name}.erp.doctype.work_order.work_order.get_default_warehouse",
				args: {
					company: frm.doc.company,
				},
				callback: function (r) {
	...
	make_se: function (frm, purpose, qty, is_additional_transfer_entry) {
		if (qty) {
			frappe
				.xcall("{app_name}.erp.doctype.work_order.work_order.make_stock_entry", {
					work_order_id: frm.doc.name,
					purpose: purpose,
					qty: qty,
	...
	make_material_request: function (frm) {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.work_order.work_order.make_material_request",
			frm,
		});
	},
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		...
		const get_pick_list = (for_qty) =>
			frappe
				.xcall("{app_name}.erp.doctype.work_order.work_order.create_pick_list", {
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.work_order.work_order.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
