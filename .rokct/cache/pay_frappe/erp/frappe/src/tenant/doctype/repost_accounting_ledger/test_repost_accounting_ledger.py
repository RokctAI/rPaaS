...
from {app_name}.erp.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries, make_purchase_receipt
from {app_name}.erp.tenant.tests.utils import ERPNextTestSuite

REPOST_MODULE = "{app_name}.erp.doctype.repost_accounting_ledger.repost_accounting_ledger"
...
class TestRepostAccountingLedger(ERPNextTestSuite):
	...
	def test_18_hook_allowed_doctype_repost(self):
		...
		with patch("{app_name}.erp.tenant.accounts.general_ledger.make_reverse_gl_entries") as make_reverse_gl_entries:
			_repost_allowed_hook_doctypes(doc, delete_cancelled_entries=False)
