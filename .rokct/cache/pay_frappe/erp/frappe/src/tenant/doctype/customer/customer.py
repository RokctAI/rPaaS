...
from {app_name}.erp.tenant.controllers.website_list_for_contact import (
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from {app_name}.erp.doctype.party_account.party_account import PartyAccount
		from {app_name}.erp.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from {app_name}.erp.doctype.sales_team.sales_team import SalesTeam
		from {app_name}.erp.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from {app_name}.erp.doctype.portal_user.portal_user import PortalUser
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "{app_name}.erp.doctype.customer.customer.send_emails",
