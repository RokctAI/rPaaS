...
frappe.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.phantom_bom_filters = {
			query: "{app_name}.erp.doctype.bom.bom.item_query",
			filters: {
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items, use_multi_level_bom) => {
				frappe.call({
					method: "{app_name}.erp.doctype.work_order.work_order.make_work_order",
					args: {
						bom_no: frm.doc.name,
						item: item,
						qty: data.qty || 0.0,
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				frappe.call({
					method: "{app_name}.erp.doctype.bom.bom.make_variant_bom",
					args: {
	...
	make_quality_inspection(frm) {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
