...
frappe.ui.form.on("Transaction Deletion Record", {
	...
	onload: function (frm) {
		if (frm.doc.docstatus == 0) {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.transaction_deletion_record.transaction_deletion_record.get_protected_doctypes",
				callback: function (r) {
					if (r.message) {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.transaction_deletion_record.transaction_deletion_record.get_doctypes_to_be_ignored",
				callback: function (r) {
			...
			frm.add_custom_button(
				...
				() => {
					new frappe.ui.FileUploader({
						...
						on_success: (file_doc) => {
							frappe.call({
								method: "{app_name}.erp.doctype.transaction_deletion_record.transaction_deletion_record.process_import_template",
								args: {
...
frappe.ui.form.on("Transaction Deletion Record To Delete", {
	doctype_name: function (frm, cdt, cdn) {
		...
		if (row.doctype_name) {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.transaction_deletion_record.transaction_deletion_record.get_company_link_fields",
				args: {
