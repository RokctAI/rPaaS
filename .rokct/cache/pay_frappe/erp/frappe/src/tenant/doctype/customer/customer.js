...
frappe.ui.form.on("Customer", {
	setup: function (frm) {
		...
		frm.make_methods = {
			Quotation: () =>
				frappe.model.open_mapped_doc({
					method: "{app_name}.erp.doctype.customer.customer.make_quotation",
					frm: frm,
				}),
			"Sales Order": () =>
			...
			Opportunity: () =>
				frappe.model.open_mapped_doc({
					method: "{app_name}.erp.doctype.customer.customer.make_opportunity",
					frm: frm,
				}),
			"Payment Entry": () =>
				frappe.model.open_mapped_doc({
					method: "{app_name}.erp.doctype.customer.customer.make_payment_entry",
					frm: frm,
				}),
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "{app_name}.erp.doctype.customer.customer.get_customer_primary",
				filters: {
					customer: doc.name,
					type: "Contact",
				},
	...
	show_party_link_dialog: function (frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				frappe.call({
					method: "{app_name}.erp.doctype.party_link.party_link.create_party_link",
					args: {
