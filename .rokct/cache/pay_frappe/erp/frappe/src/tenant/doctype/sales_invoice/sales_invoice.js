...
frappe.provide("{app_name}.erp.tenant.accounts");
...
erpnext.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			...
			if (internal) {
				...
				frappe.call({
					method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.get_received_items",
					args: {
	...
	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
				...
				let d = new frappe.ui.Dialog({
					...
					fields: [
						...
						},
						{
							...
							get_query: () => {
								return {
									query: "{app_name}.erp.tenant.controllers.queries.item_query",
									filters: {
										is_sales_item: 1,
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				erpnext.utils.map_current_doc({
					method: "{app_name}.erp.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				...
				erpnext.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "{app_name}.erp.tenant.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
					allow_child_item_selection: true,
	...
	customer() {
		...
		erpnext.utils.get_party_details(
			this.frm,
			"{app_name}.erp.tenant.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
...
cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "{app_name}.erp.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "{app_name}.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "{app_name}.erp.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "{app_name}.erp.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "{app_name}.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "{app_name}.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
