...
from {app_name}.erp.doctype.item_default.item_default import ItemDefault
from {app_name}.erp.tenant.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.item_barcode.item_barcode import ItemBarcode
		from {app_name}.erp.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from {app_name}.erp.doctype.item_default.item_default import ItemDefault
		from {app_name}.erp.doctype.item_reorder.item_reorder import ItemReorder
		from {app_name}.erp.doctype.item_supplier.item_supplier import ItemSupplier
		from {app_name}.erp.doctype.item_tax.item_tax import ItemTax
		from {app_name}.erp.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from {app_name}.erp.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
	...
	def recalculate_bin_qty(self, new_name):
		from {app_name}.erp.tenant.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"{app_name}.erp.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def get_asset_naming_series():
	from {app_name}.erp.doctype.asset.asset import get_asset_naming_series
