...
from {app_name}.erp.tenant.erp_init import get_company_currency
from {app_name}.erp.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
)
from {app_name}.erp.doctype.bank_account.bank_account import get_party_bank_account
from {app_name}.erp.doctype.payment_entry.payment_entry import (
	get_payment_entry,
)
from {app_name}.erp.doctype.subscription_plan.subscription_plan import get_plan_rate
from {app_name}.erp.tenant.accounts.party import get_party_account
from {app_name}.erp.tenant.accounts.utils import get_account_currency, get_advance_payment_doctypes, get_currency_precision
from {app_name}.erp.tenant.utilities import payment_app_import_guard
...
def _get_payment_gateway_controller(*args, **kwargs):
	# ROKCT: the upstream `payments` app is carried by the composed gateways
	# module; the import is resolved from __name__ because doctype-tree files
	# must not carry the composer's app-name token for cross-module paths
	# (crm-port precedent). The guard still catches ImportError.
	with payment_app_import_guard():
		from importlib import import_module

		get_payment_gateway_controller = import_module(
			__name__.split(".erp.doctype.", 1)[0] + ".gateways.utils"
...
class PaymentRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.payment_reference.payment_reference import PaymentReference
		from {app_name}.erp.doctype.subscription_plan_detail.subscription_plan_detail import (
	...
	def on_payment_authorized(self, payment_status=None):
		"""Gateway-controller callback (run_method) after an external payment
		attempt; bridges the gateway result to the Payment Request lifecycle
		(ROKCT reinstatement of the pre-ecommerce-split handler; upstream
		frappe/payments#204). Returns None on purpose: gateway
		finalize_request() treats a truthy return as a custom redirect URL."""
		if payment_status in ("Authorized", "Verified", "Completed"):
			if self.status != "Paid":
				self.set_as_paid()
	...
	def create_subscription(self, payment_provider, gateway_controller, data):
		if payment_provider == "stripe":
			# ROKCT: gateways-module import resolved from __name__ (crm-port
...
def make_payment_request(**args):
	...
	if args.loyalty_points and ref_doc.doctype == "Sales Order":
		from {app_name}.erp.doctype.loyalty_program.loyalty_program import validate_loyalty_points
...
def get_subscription_details(reference_doctype, reference_name):
	# ROKCT fleet-collision excision: the erpnext Subscription feature
	# (Subscription, Subscription Settings, Subscription Invoice, Process
	# Subscription) is excluded from the port — its doctype names collide
	# with the agent repo's subscriptions module (see port/port_erpnext.py
	# EXCLUDE_FLEET_DOCTYPES). Invoices are never subscription-generated in a
	# composed shell; keep the whitelisted endpoint (payment_request.js calls
