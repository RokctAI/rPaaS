	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "{app_name}.erp.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
	...
	make_supplier_quotation: function (frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "{app_name}.erp.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
frappe.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "{app_name}.erp.tenant.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
			...
			this.frm.add_custom_button(
				...
				function () {
					erpnext.utils.map_current_doc({
						method: "{app_name}.crm.doctype.opportunity.opportunity.make_request_for_quotation",
				...
				function () {
					...
					var dialog = new frappe.ui.Dialog({
						...
						primary_action: (args) => {
							...
							erpnext.utils.map_current_doc({
								method: "{app_name}.erp.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							frappe
								.call({
									method: "{app_name}.erp.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
