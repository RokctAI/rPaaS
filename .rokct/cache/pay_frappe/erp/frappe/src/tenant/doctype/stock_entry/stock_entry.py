...
from {app_name}.erp.tenant.accounts.general_ledger import process_gl_map
from {app_name}.erp.tenant.accounts.utils import get_account_currency
from {app_name}.erp.tenant.buying.utils import check_on_hold_or_closed_status
from {app_name}.erp.tenant.controllers.taxes_and_totals import init_landed_taxes_and_totals
from {app_name}.erp.doctype.bom.bom import (
	add_additional_cost,
	get_op_cost_from_sub_assemblies,
	get_secondary_items_from_sub_assemblies,
	validate_bom_no,
)
from {app_name}.erp.doctype.work_order.services.material_coverage import (
	get_minimum_material_coverage_fraction,
)
# ROKCT fleet-collision excision: the erpnext Brand master is excluded (its
# doctype name collides with the commerce repo's products module); brand-level
# item defaults are neutralized (see port/port_erpnext.py EXCLUDE_FLEET_DOCTYPES).
from {app_name}.erp.tenant.fleet_shims import get_brand_defaults
from {app_name}.erp.doctype.item_group.item_group import get_item_group_defaults
from {app_name}.erp.doctype.batch.batch import get_batch_qty
from {app_name}.erp.doctype.item.item import get_item_defaults
...
from {app_name}.erp.tenant.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from {app_name}.erp.tenant.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from {app_name}.erp.tenant.stock.utils import get_bin, get_combine_datetime, get_incoming_rate
...
from {app_name}.erp.tenant.controllers.stock_controller import StockController
from {app_name}.erp.tenant.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from {app_name}.erp.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def before_validate(self):
		from {app_name}.erp.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def check_invalid_serial_batch_nos_for_finished_good_item(self, row) -> bool:
		from {app_name}.erp.doctype.serial_no.serial_no import get_serial_nos
		from {app_name}.erp.tenant.stock.serial_batch_bundle import get_batches_from_bundle, get_serial_nos_from_bundle
	...
	def validate_source_stock_entry(self):
		...
		from {app_name}.erp.doctype.work_order.work_order import get_disassembly_available_qty
	...
	def set_actual_qty(self):
		from {app_name}.erp.tenant.stock.stock_ledger import is_negative_stock_allowed
	...
	def validate_same_source_target_warehouse_during_material_transfer(self):
		...
		if frappe.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
			from {app_name}.erp.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
	...
	def get_backflush_based_on(self):
		from {app_name}.erp.doctype.bom.bom import get_backflush_based_on
	...
	def get_bom_raw_materials(self, qty):
		from {app_name}.erp.doctype.bom.bom import get_bom_items_as_dict
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from {app_name}.erp.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from {app_name}.erp.doctype.pick_list.pick_list import update_pick_list_status
...
def get_expired_batch_items():
	from {app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
