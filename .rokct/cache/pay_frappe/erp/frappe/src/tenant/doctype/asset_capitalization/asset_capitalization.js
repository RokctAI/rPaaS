...
frappe.provide("{app_name}.erp.tenant.assets");
...
erpnext.assets.AssetCapitalization = class AssetCapitalization extends erpnext.stock.StockController {
	...
	set_consumed_stock_items_tagged_to_wip_composite_asset(target_asset) {
		...
		if (target_asset) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_items_tagged_to_wip_composite_asset",
				args: {
					params: {
	...
	get_target_item_details() {
		...
		if (me.frm.doc.target_item_code) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_target_item_details",
				args: {
	...
	get_target_asset_details() {
		...
		if (me.frm.doc.target_asset) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_target_asset_details",
				args: {
	...
	get_consumed_stock_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_consumed_stock_item_details",
				child: row,
				args: {
					ctx: {
						item_code: row.item_code,
	...
	get_consumed_asset_details(row) {
		...
		if (row && row.asset) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_consumed_asset_details",
				child: row,
				args: {
					ctx: {
						asset: row.asset,
	...
	get_service_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_service_item_details",
				child: row,
				args: {
					ctx: {
						item_code: row.item_code,
	...
	get_warehouse_details(item) {
		...
		if (item.item_code && item.warehouse) {
			me.frm.call({
				method: "{app_name}.erp.doctype.asset_capitalization.asset_capitalization.get_warehouse_details",
				child: item,
				args: {
					args: {
						item_code: item.item_code,
