...
erpnext.buying.SupplierQuotationController = class SupplierQuotationController extends (
			...
			this.frm.add_custom_button(
				...
				function () {
					...
					erpnext.utils.map_current_doc({
						...
						get_query_method:
							"{app_name}.erp.doctype.request_for_quotation.request_for_quotation.get_rfq_containing_supplier",
					});
				},
				__("Get Items From")
			);
	...
	make_quotation() {
		frappe.model.open_mapped_doc({
			method: "{app_name}.erp.doctype.supplier_quotation.supplier_quotation.make_quotation",
			frm: this.frm,
		});
	}
};
