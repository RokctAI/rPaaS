...
frappe.treeview_settings["Account"] = {
	...
	filters: [
		{
			...
			on_change: function () {
				...
				frappe.call({
					method: "{app_name}.erp.doctype.account.account.get_root_company",
					args: {
						company: company,
					},
					callback: function (r) {
	...
	root_label: "Accounts",
	get_tree_nodes: "{app_name}.erp.tenant.accounts.utils.get_children",
	on_node_render: function (node, deep) {
		...
		if (!cur_tree.account_balance_data) {
			frappe.db.get_single_value("Accounts Settings", "show_balance_in_coa").then((value) => {
				if (value) {
					frappe.call({
						method: "{app_name}.erp.tenant.accounts.utils.get_account_balances_coa",
						args: {
	...
	add_tree_node: "{app_name}.erp.tenant.accounts.utils.add_ac",
	menu_items: [
		{
