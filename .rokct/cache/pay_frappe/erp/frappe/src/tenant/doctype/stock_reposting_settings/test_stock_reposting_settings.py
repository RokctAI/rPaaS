...
from {app_name}.erp.doctype.repost_item_valuation.repost_item_valuation import get_recipients
from {app_name}.erp.doctype.stock_reposting_settings import stock_reposting_settings as srs
...
class TestStockRepostingSettings(ERPNextTestSuite):
	...
	def test_reposts_only_current_financial_year_entries(self):
		...
		calls = []
		variance_path = "{app_name}.erp.tenant.report.stock_ledger_variance.stock_ledger_variance.get_data"
		with (
			patch.object(
	...
	def test_value_comparison_excludes_je_and_flags_wrong_account(self):
		...
		sent = []
		comparison = (
			"{app_name}.erp.tenant.report.stock_and_account_value_comparison.stock_and_account_value_comparison"
		)
		with (
			patch("{app_name}.erp.tenant.erp_init.is_perpetual_inventory_enabled", return_value=True),
