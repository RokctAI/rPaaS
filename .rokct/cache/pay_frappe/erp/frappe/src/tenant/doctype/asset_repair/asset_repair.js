...
frappe.ui.form.on("Asset Repair", {
	setup: function (frm) {
		...
		frm.set_query("purchase_invoice", "invoices", function () {
			return {
				query: "{app_name}.erp.doctype.asset_repair.asset_repair.get_purchase_invoice",
				filters: {
					company: frm.doc.company,
				},
			};
		...
		frm.set_query("expense_account", "invoices", function (doc, cdt, cdn) {
			...
			return {
				query: "{app_name}.erp.doctype.asset_repair.asset_repair.get_expense_accounts",
				filters: {
	...
	set_downtime: (frm) => {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.asset_repair.asset_repair.get_downtime",
			args: {
...
frappe.ui.form.on("Asset Repair Purchase Invoice", {
	...
	expense_account: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.asset_repair.asset_repair.get_unallocated_repair_cost",
			args: {
				purchase_invoice: row.purchase_invoice,
