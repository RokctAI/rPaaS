...
frappe.ui.form.on("Item", {
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(erpnext.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "{app_name}.erp.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				erpnext.item.item_dashboard = new erpnext.stock.ItemDashboard({
					...
					item_code: frm.doc.name,
					page_length: 20,
					method: "{app_name}.erp.tenant.stock.dashboard.item_dashboard.get_data",
	...
	render_item_prices: function (frm) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.item.item.get_item_prices",
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "{app_name}.erp.tenant.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "{app_name}.erp.tenant.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "{app_name}.erp.tenant.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "{app_name}.erp.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "{app_name}.erp.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
					stock_uom: frm.doc.stock_uom,
				},
