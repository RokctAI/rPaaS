...
frappe.ui.form.on("Payment Request", "onload", function (frm, dt, dn) {
	if (frm.doc.reference_doctype) {
		frappe.call({
			method: "{app_name}.erp.doctype.payment_request.payment_request.get_print_format_list",
...
frappe.ui.form.on("Payment Request", "refresh", function (frm) {
	...
	if (
		...
		frm.add_custom_button(__("Create Payment Entry"), function () {
			frappe.call({
				method: "{app_name}.erp.doctype.payment_request.payment_request.make_payment_entry",
...
frappe.ui.form.on("Payment Request", "is_a_subscription", function (frm) {
	...
	if (frm.doc.is_a_subscription && frm.doc.reference_doctype && frm.doc.reference_name) {
		frappe.call({
			method: "{app_name}.erp.doctype.payment_request.payment_request.get_subscription_details",
