...
class ProcessPeriodClosingVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.process_period_closing_voucher_detail.process_period_closing_voucher_detail import (
	...
	if normal_balances := (
		...
		if not is_scheduler_inactive():
			for x in normal_balances:
				...
				frappe.enqueue(
					method="{app_name}.erp.doctype.process_period_closing_voucher.process_period_closing_voucher.process_individual_date",
					queue="long",
					timeout=timeout,
					is_async=True,
...
def schedule_next_date(docname: str):
	...
	if to_process := (
		...
		if total_no_of_dates == completed:
			from {app_name}.erp.doctype.process_payment_reconciliation.process_payment_reconciliation import (
				is_job_running,
			)
			...
			if not is_job_running(job_name):
				frappe.enqueue(
					method="{app_name}.erp.doctype.process_period_closing_voucher.process_period_closing_voucher.summarize_and_post_ledger_entries",
					queue="long",
					timeout=timeout,
					is_async=True,
					job_name=job_name,
