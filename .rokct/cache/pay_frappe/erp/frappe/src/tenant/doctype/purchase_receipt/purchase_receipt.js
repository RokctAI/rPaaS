...
frappe.ui.form.on("Purchase Receipt", {
	...
	refresh: function (frm) {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
						frm: cur_frm,
					});
				},
				__("Create")
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_inter_company_delivery_note",
						frm: cur_frm,
					});
				},
				__("Create")
	...
	make_lcv(frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_lcv",
			args: {
			...
			frm.add_custom_button(
				...
				function () {
					...
					erpnext.utils.map_current_doc({
						method: "{app_name}.erp.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
			...
			frappe.prompt(
				...
				function (values) {
					if (values.return_for_rejected_warehouse) {
						frappe.call({
							method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_purchase_return_against_rejected_warehouse",
							args: {
...
cur_frm.cscript.update_status = function (status) {
	...
	frappe.call({
		method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.update_purchase_receipt_status",
...
cur_frm.cscript._make_purchase_return = function () {
	frappe.model.open_mapped_doc({
		method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_purchase_return",
		frm: cur_frm,
	});
};
...
cur_frm.cscript["Make Stock Entry"] = function () {
	frappe.model.open_mapped_doc({
		method: "{app_name}.erp.doctype.purchase_receipt.purchase_receipt.make_stock_entry",
		frm: cur_frm,
	});
};
