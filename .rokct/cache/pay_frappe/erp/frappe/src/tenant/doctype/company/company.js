...
frappe.ui.form.on("Company", {
	...
	delete_company_transactions: function (frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
					...
					frappe.verify_password(function () {
						var d = frappe.prompt(
							...
							function (data) {
								...
								frappe.call({
									method: "{app_name}.erp.doctype.company.company.create_transaction_deletion_request",
									args: {
...
erpnext.company.set_chart_of_accounts_options = function (doc) {
	...
	if (doc.country) {
		return frappe.call({
			method: "{app_name}.erp.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
