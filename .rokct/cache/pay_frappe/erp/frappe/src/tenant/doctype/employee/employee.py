...
from {app_name}.erp.tenant.utilities.transaction_base import delete_events
...
class Employee(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from {app_name}.erp.doctype.employee_education.employee_education import EmployeeEducation
		from {app_name}.erp.doctype.employee_external_work_history.employee_external_work_history import (
			EmployeeExternalWorkHistory,
		)
		from {app_name}.erp.doctype.employee_internal_work_history.employee_internal_work_history import (
	...
	def autoname(self):
		# ROKCT fix(hr): honor HR Settings' "Employee Naming By" choice (rhrms
		# rewire — upstream hrms's EmployeeMaster.autoname, baked into the fork;
		# see port/port_erpnext.py). Falls back to the stock naming series when
		# the hrms module is not composed or the setting is unset.
		naming_method = None
	...
	def validate(self):
		from {app_name}.erp.tenant.controllers.status_updater import validate_status
	...
	def validate_sa_id_number(self):
		# ROKCT fix(hr): South African 13-digit ID validation — length, Luhn
		# checksum and an 18-years-minimum age derived from the encoded date
		...
		checksum = 0
		for i, digit in enumerate(cint(d) for d in reversed(self.id_number)):
			if i % 2 == 1:
				digit *= 2
				if digit > 9:
					digit -= 9
		...
		if dob and date_diff(nowdate(), dob) < 18 * 365.25:
			frappe.throw(
				_("Employee must be at least 18 years old"), title=_("Age Restriction")
			)

	def get_dob_from_id_number(self):
		"""Date of birth encoded in a South African ID number (YYMMDD prefix)."""
		from datetime import date

		if not self.get("id_number") or len(self.id_number) < 6:
			return None

		yy, mm, dd = self.id_number[:2], self.id_number[2:4], self.id_number[4:6]
		century = 1900 if cint(yy) > cint(nowdate()[:4]) % 100 else 2000
		try:
	...
	def validate_sa_bank_details(self):
		# ROKCT fix(hr): SA bank detail sanity checks (rhrms rewire; see
		# port/port_erpnext.py). Upstream's bank_ac_no/iban are not touched.
	...
	def is_sa_company(self):
		"""ROKCT (rhrms rewire): gate the SA-specific HR hardening on the
		employee's company being South African — rhrms ran the checks
		unconditionally in an SA-only deployment; the fork must stay correct
		for other markets."""
		if not self.company:
			return False
		return (frappe.get_cached_value("Company", self.company, "country") == "South Africa")
