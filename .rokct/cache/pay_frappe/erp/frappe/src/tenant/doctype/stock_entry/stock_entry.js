...
frappe.provide("{app_name}.erp.tenant.stock");
frappe.provide("{app_name}.erp.tenant.accounts.dimensions");
...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("source_warehouse_address", function () {
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_warehouse_address",
				filters: {
		...
		frm.set_query("project", "items", function (doc) {
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_project_name",
				filters: {
					company: doc.company,
				},
			};
		...
		if (frm.doc.purpose == "Receive from Customer") {
			frm.set_query("against_fg", "items", function () {
				return {
					query: "{app_name}.erp.tenant.controllers.subcontracting_inward_controller.get_fg_reference_names",
					filters: {
	...
	setup_quality_inspection: function (frm) {
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "{app_name}.erp.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	source_stock_entry: async function (frm) {
		...
		let available_qty = await frappe.xcall(
			"{app_name}.erp.doctype.work_order.work_order.get_disassembly_available_qty",
	...
	refresh: function (frm) {
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					frappe.model.open_mapped_doc({
						method: "{app_name}.erp.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
							...
							async (data) => {
								if (frm.doc.work_order) {
									let stock_entry = await frappe.xcall(
										"{app_name}.erp.doctype.work_order.work_order.make_stock_entry",
										{
			...
			frm.add_custom_button(
				...
				function () {
					erpnext.utils.map_current_doc({
						method: "{app_name}.erp.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = erpnext.utils.map_current_doc({
						method: "{app_name}.erp.doctype.material_request.material_request.make_stock_entry",
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "{app_name}.erp.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		frappe
			.call({
				method: "{app_name}.erp.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
	...
	make_retention_stock_entry: function (frm) {
		frappe.call({
			method: "{app_name}.erp.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			frappe.call({
				method: "{app_name}.erp.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			frappe.call({
				method: "{app_name}.erp.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			erpnext.utils.map_current_doc({
				method: "{app_name}.erp.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return frappe.call({
				method: "{app_name}.erp.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && d.transfer_qty && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: "{app_name}.erp.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
erpnext.stock.StockEntry = class StockEntry extends erpnext.stock.StockController {
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "{app_name}.erp.tenant.accounts.utils.get_company_default",
			args: {
	...
	work_order() {
		...
		return frappe.call({
			method: "{app_name}.erp.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
