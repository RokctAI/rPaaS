	...
	filters: [
		...
		},
		{
			...
			get_query: () => {
				...
				if (quote != "") {
					return {
						query: "{app_name}.erp.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
	...
	make_default_supplier_dialog: (report) => {
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				frappe.call({
					method: "{app_name}.erp.tenant.report.supplier_quotation_comparison.supplier_quotation_comparison.set_default_supplier",
					args: {
