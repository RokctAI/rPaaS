...
from {app_name}.erp.doctype.account.account import RootNotEditable
from {app_name}.erp.tenant.regional.address_template.setup import set_up_address_templates
from {app_name}.erp.tenant.setup.utils import identity as _
...
def install(country=None):
	...
	base_path = frappe.get_app_path("{app_name}", "erp", "doctype")
	...
	from {app_name}.erp.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(frappe.get_app_path("{app_name}", "erp", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
	...
	uom_conversions = json.loads(
		open(
			frappe.get_app_path("{app_name}", "erp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
