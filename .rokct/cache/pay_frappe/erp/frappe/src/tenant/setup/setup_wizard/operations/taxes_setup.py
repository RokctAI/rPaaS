...
def update_regional_tax_settings(country, company):
	path = frappe.get_app_path("{app_name}", "erp", "regional", frappe.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"{app_name}.erp.tenant.regional.{frappe.scrub(country)}.setup.update_regional_tax_settings"
