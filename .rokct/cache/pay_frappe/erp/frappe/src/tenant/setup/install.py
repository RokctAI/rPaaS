...
from {app_name}.erp.doctype.incoterm.incoterm import create_incoterms
...
def create_letter_head():
	base_path = frappe.get_app_path("{app_name}", "erp", "accounts", "letterhead")
...
def toggle_hidden_fields():
	from {app_name}.erp.doctype.accounts_settings.accounts_settings import (
...
def after_app_install(app_name=None):
	if app_name == "crm":
		from {app_name}.crm.tenant.frappe_crm_api import remove_allowed_users_on_crm_install
...
def after_app_uninstall(app_name=None):
	if app_name == "crm":
		from {app_name}.crm.tenant.frappe_crm_api import disable_frappe_crm_data_synchronization_on_crm_uninstall
