erpnext.PointOfSale.Controller = class {
	...
	fetch_opening_entry() {
		return frappe.call("{app_name}.erp.tenant.page.point_of_sale.point_of_sale.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: async function ({ company, pos_profile, balance_details }) {
				...
				const method = "{app_name}.erp.tenant.page.point_of_sale.point_of_sale.create_opening_voucher";
	...
	async prepare_app_defaults(data) {
		...
		frappe.call({
			method: "{app_name}.erp.tenant.page.point_of_sale.point_of_sale.get_pos_profile_data",
		...
		return frappe.call({
			method:
				doc.doctype == "POS Invoice"
					? "{app_name}.erp.doctype.pos_invoice.pos_invoice.make_sales_return"
					: "{app_name}.erp.doctype.sales_invoice.sales_invoice.make_sales_return",
			args: {
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		const method = "{app_name}.erp.doctype.serial_no.serial_no.get_pos_reserved_serial_nos";
	...
	get_available_stock(item_code, warehouse) {
		const me = this;
		return frappe.call({
			method: "{app_name}.erp.doctype.pos_invoice.pos_invoice.get_stock_availability",
			args: {
				item_code: item_code,
				warehouse: warehouse,
			},
