...
frappe.provide("{app_name}.erp.tenant.selling");
		...
		erpnext.selling.SellingController = class SellingController extends erpnext.TransactionController {
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "{app_name}.erp.doctype.company.company.get_default_company_address",
						args: {
			...
			project(doc, cdt, cdn) {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "{app_name}.projects.doctype.project.project.get_cost_center_name",
