...
$.extend(erpnext.queries, {
	...
	lead: function () {
		return { query: "{app_name}.erp.tenant.controllers.queries.lead_query" };
	},

	item: function (filters) {
		var args = { query: "{app_name}.erp.tenant.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "{app_name}.erp.tenant.controllers.queries.bom" };
	},

	task: function () {
		return { query: "{app_name}.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "{app_name}.erp.tenant.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "{app_name}.erp.tenant.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
erpnext.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		...
		if (row.item_code) {
			$.extend(filters, { query: "{app_name}.erp.tenant.controllers.queries.warehouse_query" });
