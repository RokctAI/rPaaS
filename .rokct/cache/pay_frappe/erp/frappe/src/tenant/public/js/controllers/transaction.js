...
erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
	setup() {
		...
		frappe.ui.form.on(this.frm.doctype + " Item", "rate", function (frm, cdt, cdn) {
			...
			if (item.item_code && item.rate) {
				frappe.call({
					method: "{app_name}.erp.tenant.stock.get_item_details.get_item_tax_template",
					args: {
						ctx: {
		...
		if (this.frm.fields_dict["items"].grid.get_field("uom")) {
			this.frm.set_query("uom", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "{app_name}.erp.tenant.controllers.queries.get_item_uom_query",
					filters: {
		...
		if (this.frm.fields_dict["items"].grid.get_field("blanket_order")) {
			this.frm.set_query("blanket_order", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "{app_name}.erp.tenant.controllers.queries.get_blanket_orders",
					filters: {
						company: doc.company,
	...
	toggle_enable_for_stock_uom(field) {
		frappe.call({
			method: "{app_name}.erp.doctype.stock_settings.stock_settings.get_enable_stock_uom_editing",
			callback: (r) => {
				if (r.message) {
	...
	make_payment_request() {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	make_payment_request_with_schedule = async function () {
		...
		const { message: schedules } = await frappe.call({
			method: "{app_name}.erp.doctype.payment_request.payment_request.get_available_payment_schedules",
			args: {
	...
	apply_default_taxes() {
		...
		if (taxes_and_charges_field) {
			return frappe.call({
				method: "{app_name}.erp.tenant.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
		...
		if (item.item_code || item.serial_no) {
			if (!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "{app_name}.erp.tenant.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						ctx: {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		frappe.call({
			method: "{app_name}.erp.tenant.stock.utils.get_incoming_rate",
			args: {
				args: item_args,
			},
			callback: function (r) {
		...
		var set_party_account = function (set_pricing) {
			if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
				...
				if (
					...
					return frappe.call({
						method: "{app_name}.erp.tenant.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if (
				...
				return frappe.call({
					method: "{app_name}.erp.tenant.accounts.party.get_due_date",
					args: {
	...
	get_exchange_rate(transaction_date, from_currency, to_currency, callback) {
		...
		return frappe.call({
			method: "{app_name}.erp.tenant.setup.utils.get_exchange_rate",
			args: {
	...
	uom(doc, cdt, cdn) {
		...
		if (item.item_code && item.uom) {
			return this.frm.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom,
				},
	...
	service_start_date(frm, cdt, cdn) {
		...
		if (child.service_start_date) {
			frappe.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.calculate_service_end_date",
				args: { ctx: child },
				callback: function (r) {
	...
	batch_no(frm, cdt, cdn) {
		...
		if (row.batch_no) {
			...
			frappe.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.get_batch_based_item_price",
				args: {
					pctx: params,
					item_code: row.item_code,
				},
	...
	ignore_pricing_rule() {
		if (this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "{app_name}.erp.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules) {
			let me = this;
			return this.frm.call({
				method: "{app_name}.erp.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "{app_name}.erp.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		return this.frm
			.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.apply_price_list",
	...
	taxes_and_charges() {
		...
		if (this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "{app_name}.erp.tenant.controllers.accounts_controller.get_taxes_and_charges",
				args: {
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.get_item_tax_info",
				args: {
					doc: me.frm.doc,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if (item.item_tax_template) {
			return this.frm.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.get_item_tax_map",
				args: {
					doc: me.frm.doc,
	...
	make_quality_inspection() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				frappe.call({
					method: "{app_name}.erp.tenant.controllers.stock_controller.make_quality_inspections",
					args: {
						company: me.frm.doc.company,
						doctype: me.frm.doc.doctype,
		...
		frappe.call({
			method: "{app_name}.erp.tenant.controllers.stock_controller.check_item_quality_inspection",
			args: {
	...
	get_method_for_payment() {
		let method = "{app_name}.erp.doctype.payment_entry.payment_entry.get_payment_entry";
		if (this.frm.doc.__onload && this.frm.doc.__onload.make_payment_via_journal_entry) {
			if (["Sales Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
				method =
					"{app_name}.erp.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			} else {
				method =
					"{app_name}.erp.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			...
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_batch_no",
				filters: filters,
			};
		}
	}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			...
			return {
				query: "{app_name}.erp.tenant.controllers.queries.get_tax_template",
				filters: filters,
			};
		}
	}
	...
	payment_terms_template() {
		...
		if (doc.payment_terms_template && doc.doctype !== "Delivery Note" && !doc.is_return) {
			...
			frappe.call({
				method: "{app_name}.erp.tenant.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if (row.payment_term) {
			frappe.call({
				method: "{app_name}.erp.tenant.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype == "Sales Order" || item.parenttype == "Purchase Order")) {
			frappe.call({
				method: "{app_name}.erp.tenant.stock.get_item_details.get_blanket_order_details",
				args: {
					ctx: {
						item_code: item.item_code,
	...
	setup_accounting_dimension_triggers() {
		frappe.call({
			method: "{app_name}.erp.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
...
erpnext.apply_putaway_rule = (frm, purpose = null) => {
	...
	frappe.call({
		method: "{app_name}.erp.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
			company: frm.doc.company,
