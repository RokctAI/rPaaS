erpnext.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
	...
	get_dialog_fields() {
		...
		fields.push({
			...
			get_query: () => {
				return {
					query: "{app_name}.erp.tenant.controllers.queries.warehouse_query",
					filters: [
	...
	create_serial_nos() {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.create_serial_nos",
			args: {
				item_code: this.item.item_code,
	...
	upload_csv_file() {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file",
			args: {
				item_code: this.item.item_code,
				file_path: file_path,
			},
	...
	get_batch_qty(batch_no, callback) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.batch.batch.get_batch_qty",
			args: {
				batch_no: batch_no,
				warehouse: warehouse,
				item_code: this.item.item_code,
	...
	get_auto_data() {
		...
		if (qty) {
			frappe.call({
				method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data",
				args: {
					item_code: this.item.item_code,
					warehouse: warehouse,
	...
	scan_barcode_data() {
		...
		if (scan_serial_no || scan_batch_no) {
			frappe.call({
				method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_serial_batch_no_exists",
				args: {
					item_code: this.item.item_code,
	...
	update_serial_batch_no() {
		...
		if (scan_serial_no) {
			...
			if (!this.item.has_batch_no) {
				...
				frappe.call({
					method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_batch_no_from_serial_no",
					args: {
						serial_no: scan_serial_no,
					},
					callback: (r) => {
	...
	create_bundle_entries(entries, warehouse) {
		frappe
			.call({
				method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers",
				args: {
					entries: entries,
					child_row: this.item,
					doc: this.frm.doc,
	...
	render_data() {
		if (this.bundle || (this.frm.doc.is_return && this.frm.doc.return_against)) {
			frappe
				.call({
					method: "{app_name}.erp.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers",
					args: {
						item_code: this.item.item_code,
						name: this.bundle,
