		...
		erpnext.buying.BuyingController = class BuyingController extends erpnext.TransactionController {
			...
			setup_queries(doc, cdt, cdn) {
				...
				this.frm.set_query("manufacturer", "items", function (doc, cdt, cdn) {
					...
					return {
						query: "{app_name}.erp.tenant.controllers.queries.item_manufacturer_query",
			...
			company() {
				...
				frappe.call({
					method: "{app_name}.erp.doctype.company.company.get_billing_shipping_address",
					args: {
			...
			warehouse(doc, cdt, cdn) {
				...
				if (item.item_code && item.warehouse) {
					return this.frm.call({
						method: "{app_name}.erp.tenant.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			manufacturer(doc, cdt, cdn) {
				...
				if (row.manufacturer) {
					frappe.call({
						method: "{app_name}.erp.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
...
erpnext.buying.get_default_bom = function (frm) {
	$.each(frm.doc["items"] || [], function (i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "{app_name}.erp.tenant.stock.get_item_details.get_default_bom",
				args: {
...
erpnext.buying.get_items_from_product_bundle = function (frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args) {
			...
			return frappe.call({
				type: "GET",
				method: "{app_name}.erp.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
