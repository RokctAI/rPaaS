...
frappe.provide("{app_name}.erp.tenant.stock.utils");
...
$.extend(erpnext.utils, {
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "{app_name}.erp.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "{app_name}.erp.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "{app_name}.erp.tenant.accounts.utils.get_fiscal_year",
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "{app_name}.erp.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
erpnext.utils.update_child_items = function (opts) {
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "{app_name}.erp.tenant.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "{app_name}.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
