...
$.extend(erpnext.stock_reservation, {
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"{app_name}.erp.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"{app_name}.erp.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "{app_name}.erp.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"{app_name}.erp.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
