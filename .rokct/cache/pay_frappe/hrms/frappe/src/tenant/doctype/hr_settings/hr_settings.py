...
class HRSettings(Document):
	...
	def set_naming_series(self):
		from {app_name}.erp.tenant.utilities.naming import set_by_naming_series
	...
	def validate_frequency_change(self):
		...
		try:
			weekly_job = frappe.get_doc(
				"Scheduled Job Type",
				{"method": "{app_name}.hrms.tenant.controllers.employee_reminders.send_reminders_in_advance_weekly"},
			)

			monthly_job = frappe.get_doc(
				"Scheduled Job Type",
				{"method": "{app_name}.hrms.tenant.controllers.employee_reminders.send_reminders_in_advance_monthly"},
			)
