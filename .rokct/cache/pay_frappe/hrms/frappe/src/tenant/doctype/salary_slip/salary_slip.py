...
from {app_name}.erp.doctype.employee.employee import get_holiday_list_for_employee
from {app_name}.erp.tenant.utilities.transaction_base import TransactionBase

from {app_name}.hrms.tenant import hrms_init as hrms
from {app_name}.hrms.tenant.hr.utils import validate_active_employee
from {app_name}.hrms.doctype.additional_salary.additional_salary import get_additional_salaries
from {app_name}.hrms.doctype.employee_benefit_ledger.employee_benefit_ledger import (
	create_employee_benefit_ledger_entry,
	delete_employee_benefit_ledger_entry,
)
from {app_name}.hrms.doctype.income_tax_slab.income_tax_slab import calculate_tax_by_tax_slab
from {app_name}.hrms.doctype.payroll_entry.payroll_entry import get_salary_withholdings, get_start_end_dates
from {app_name}.hrms.doctype.payroll_period.payroll_period import (
	get_payroll_period,
	get_period_factor,
)
from {app_name}.hrms.doctype.salary_slip.salary_slip_loan_utils import (
	cancel_loan_repayment_entry,
	make_loan_repayment_entry,
	process_loan_interest_accrual_and_demand,
	set_loan_repayment,
)
from {app_name}.hrms.tenant.payroll.utils import (
	COMPONENT_EVAL_GLOBALS,
	_safe_eval,
	get_component_eval_context,
	throw_error_message,
)
from {app_name}.hrms.tenant.utils.holiday_list import get_holiday_dates_between
...
class SalarySlip(TransactionBase):
	...
	def add_leave_balances(self):
		...
		if frappe.db.get_single_value("Payroll Settings", "show_leave_balances_in_salary_slip"):
			from {app_name}.hrms.doctype.leave_application.leave_application import get_leave_details
...
def enqueue_email_salary_slips(names: list | str) -> None:
	...
	frappe.enqueue("{app_name}.hrms.doctype.salary_slip.salary_slip.email_salary_slips", names=names)
	frappe.msgprint(
