...
frappe.provide("{app_name}.hrms.tenant.hr");
...
frappe.ui.form.on("Expense Claim", {
	setup: function (frm) {
		...
		frm.set_query("expense_approver", function () {
			return {
				query: "{app_name}.hrms.doctype.department_approver.department_approver.get_approvers",
				filters: {
					employee: frm.doc.employee,
	...
	onload: function (frm) {
		...
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "{app_name}.hrms.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function (r) {
	...
	make_payment_entry: function (frm) {
		return frappe.call({
			method: "{app_name}.hrms.tenant.overrides.employee_payment_entry.get_payment_entry_for_employee",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
	...
	company: function (frm) {
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			frappe.call({
				method: "{app_name}.hrms.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
				args: {
	...
	get_advances: function (frm) {
		if (frm.doc.employee) {
			return frappe.call({
				method: "{app_name}.hrms.doctype.expense_claim.expense_claim.get_advances",
				args: {
