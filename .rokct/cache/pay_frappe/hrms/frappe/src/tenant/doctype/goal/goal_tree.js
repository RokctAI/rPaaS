...
frappe.treeview_settings["Goal"] = {
	get_tree_nodes: "{app_name}.hrms.doctype.goal.goal.get_children",
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
	...
	fields: [
		...
		},
		{
			...
			get_query() {
				return {
					query: "{app_name}.hrms.doctype.appraisal.appraisal.get_kras_for_employee",
					filters: {
	...
	get_tree_root: false,
	add_tree_node: "{app_name}.hrms.doctype.goal.goal.add_tree_node",
...
function update_progress(node, progress) {
	return frappe
		.call({
			method: "{app_name}.hrms.doctype.goal.goal.update_progress",
			args: {
