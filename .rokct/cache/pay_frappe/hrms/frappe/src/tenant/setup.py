# compliance-ignore-file: path-traversal (install-time fixture load; joins the app path with constant email-template paths)
import os
...
from {app_name}.hrms.tenant.overrides.company import delete_company_fixtures
...
def setup_notifications():
	base_path = frappe.get_app_path("{app_name}", "hrms", "doctype")
...
def run_post_install_patches():
	...
	try:
		for patch in POST_INSTALL_PATCHES:
			...
			# ROKCT: post-install patches are ported to the patches_post_install
			# package (the composer redirects src/patches app-level; see
			# port/port_hrms.py).
			frappe.get_attr(f"{app_name}.hrms.tenant.patches_post_install.{patch_name}.execute")()
	finally:
